...
interface Props {
  participante: Participante[]; // Lista de participantes
...
export const HorizontalCarrusel = ({
  ...
  }, [participante]); // Este efecto se ejecuta cuando participante cambia
  ...
  return (
    <View style={{ height: nombre ? 260 : 220 }}>
      ...
      <FlatList
        data={participante} // Lista de participantes
        renderItem={(
          { item } // Renderiza cada participante utilizando el componente ParticipantePoster
        ) => (
          <ParticipantePoster participante={item} width={140} height={200} />
        )}
        keyExtractor={(item, index) => `${item.id}-${index}`} // Función para generar claves únicas para los elementos de la lista
